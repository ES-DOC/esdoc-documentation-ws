"""A set of cim 1.5 packages.

CIM CODE GENERATOR :: Code generated @ 2013-01-30 15:45:18.472829.
"""

# Module imports.
from esdoc_api.lib.pycim.v1_5.types.activity.activity import Activity
from esdoc_api.lib.pycim.v1_5.types.activity.boundary_condition import BoundaryCondition
from esdoc_api.lib.pycim.v1_5.types.activity.conformance import Conformance
from esdoc_api.lib.pycim.v1_5.types.activity.ensemble import Ensemble
from esdoc_api.lib.pycim.v1_5.types.activity.ensemble_member import EnsembleMember
from esdoc_api.lib.pycim.v1_5.types.activity.experiment import Experiment
from esdoc_api.lib.pycim.v1_5.types.activity.experiment_relationship import ExperimentRelationship
from esdoc_api.lib.pycim.v1_5.types.activity.experiment_relationship_target import ExperimentRelationshipTarget
from esdoc_api.lib.pycim.v1_5.types.activity.initial_condition import InitialCondition
from esdoc_api.lib.pycim.v1_5.types.activity.measurement_campaign import MeasurementCampaign
from esdoc_api.lib.pycim.v1_5.types.activity.numerical_activity import NumericalActivity
from esdoc_api.lib.pycim.v1_5.types.activity.numerical_experiment import NumericalExperiment
from esdoc_api.lib.pycim.v1_5.types.activity.numerical_requirement import NumericalRequirement
from esdoc_api.lib.pycim.v1_5.types.activity.output_requirement import OutputRequirement
from esdoc_api.lib.pycim.v1_5.types.activity.physical_modification import PhysicalModification
from esdoc_api.lib.pycim.v1_5.types.activity.requirement_option import RequirementOption
from esdoc_api.lib.pycim.v1_5.types.activity.simulation import Simulation
from esdoc_api.lib.pycim.v1_5.types.activity.simulation_composite import SimulationComposite
from esdoc_api.lib.pycim.v1_5.types.activity.simulation_relationship import SimulationRelationship
from esdoc_api.lib.pycim.v1_5.types.activity.simulation_relationship_target import SimulationRelationshipTarget
from esdoc_api.lib.pycim.v1_5.types.activity.simulation_run import SimulationRun
from esdoc_api.lib.pycim.v1_5.types.activity.spatio_temporal_constraint import SpatioTemporalConstraint
from esdoc_api.lib.pycim.v1_5.types.data.data_content import DataContent
from esdoc_api.lib.pycim.v1_5.types.data.data_distribution import DataDistribution
from esdoc_api.lib.pycim.v1_5.types.data.data_extent import DataExtent
from esdoc_api.lib.pycim.v1_5.types.data.data_extent_geographical import DataExtentGeographical
from esdoc_api.lib.pycim.v1_5.types.data.data_extent_temporal import DataExtentTemporal
from esdoc_api.lib.pycim.v1_5.types.data.data_extent_time_interval import DataExtentTimeInterval
from esdoc_api.lib.pycim.v1_5.types.data.data_hierarchy_level import DataHierarchyLevel
from esdoc_api.lib.pycim.v1_5.types.data.data_object import DataObject
from esdoc_api.lib.pycim.v1_5.types.data.data_property import DataProperty
from esdoc_api.lib.pycim.v1_5.types.data.data_restriction import DataRestriction
from esdoc_api.lib.pycim.v1_5.types.data.data_storage import DataStorage
from esdoc_api.lib.pycim.v1_5.types.data.data_storage_db import DataStorageDb
from esdoc_api.lib.pycim.v1_5.types.data.data_storage_file import DataStorageFile
from esdoc_api.lib.pycim.v1_5.types.data.data_storage_ip import DataStorageIp
from esdoc_api.lib.pycim.v1_5.types.data.data_topic import DataTopic
from esdoc_api.lib.pycim.v1_5.types.grids.coordinate_list import CoordinateList
from esdoc_api.lib.pycim.v1_5.types.grids.grid_extent import GridExtent
from esdoc_api.lib.pycim.v1_5.types.grids.grid_mosaic import GridMosaic
from esdoc_api.lib.pycim.v1_5.types.grids.grid_property import GridProperty
from esdoc_api.lib.pycim.v1_5.types.grids.grid_spec import GridSpec
from esdoc_api.lib.pycim.v1_5.types.grids.grid_tile import GridTile
from esdoc_api.lib.pycim.v1_5.types.grids.grid_tile_resolution_type import GridTileResolutionType
from esdoc_api.lib.pycim.v1_5.types.grids.vertical_coordinate_list import VerticalCoordinateList
from esdoc_api.lib.pycim.v1_5.types.quality.cim_quality import CimQuality
from esdoc_api.lib.pycim.v1_5.types.quality.evaluation import Evaluation
from esdoc_api.lib.pycim.v1_5.types.quality.measure import Measure
from esdoc_api.lib.pycim.v1_5.types.quality.report import Report
from esdoc_api.lib.pycim.v1_5.types.shared.calendar import Calendar
from esdoc_api.lib.pycim.v1_5.types.shared.change import Change
from esdoc_api.lib.pycim.v1_5.types.shared.change_property import ChangeProperty
from esdoc_api.lib.pycim.v1_5.types.shared.cim_document_relationship import CimDocumentRelationship
from esdoc_api.lib.pycim.v1_5.types.shared.cim_document_relationship_target import CimDocumentRelationshipTarget
from esdoc_api.lib.pycim.v1_5.types.shared.cim_genealogy import CimGenealogy
from esdoc_api.lib.pycim.v1_5.types.shared.cim_info import CimInfo
from esdoc_api.lib.pycim.v1_5.types.shared.cim_reference import CimReference
from esdoc_api.lib.pycim.v1_5.types.shared.cim_relationship import CimRelationship
from esdoc_api.lib.pycim.v1_5.types.shared.cim_type_info import CimTypeInfo
from esdoc_api.lib.pycim.v1_5.types.shared.citation import Citation
from esdoc_api.lib.pycim.v1_5.types.shared.closed_date_range import ClosedDateRange
from esdoc_api.lib.pycim.v1_5.types.shared.compiler import Compiler
from esdoc_api.lib.pycim.v1_5.types.shared.daily_360 import Daily360
from esdoc_api.lib.pycim.v1_5.types.shared.data_source import DataSource
from esdoc_api.lib.pycim.v1_5.types.shared.date_range import DateRange
from esdoc_api.lib.pycim.v1_5.types.shared.license import License
from esdoc_api.lib.pycim.v1_5.types.shared.machine import Machine
from esdoc_api.lib.pycim.v1_5.types.shared.machine_compiler_unit import MachineCompilerUnit
from esdoc_api.lib.pycim.v1_5.types.shared.open_date_range import OpenDateRange
from esdoc_api.lib.pycim.v1_5.types.shared.perpetual_period import PerpetualPeriod
from esdoc_api.lib.pycim.v1_5.types.shared.platform import Platform
from esdoc_api.lib.pycim.v1_5.types.shared.property import Property
from esdoc_api.lib.pycim.v1_5.types.shared.real_calendar import RealCalendar
from esdoc_api.lib.pycim.v1_5.types.shared.responsible_party import ResponsibleParty
from esdoc_api.lib.pycim.v1_5.types.shared.responsible_party_contact_info import ResponsiblePartyContactInfo
from esdoc_api.lib.pycim.v1_5.types.shared.standard import Standard
from esdoc_api.lib.pycim.v1_5.types.shared.standard_name import StandardName
from esdoc_api.lib.pycim.v1_5.types.software.component import Component
from esdoc_api.lib.pycim.v1_5.types.software.component_language import ComponentLanguage
from esdoc_api.lib.pycim.v1_5.types.software.component_language_property import ComponentLanguageProperty
from esdoc_api.lib.pycim.v1_5.types.software.component_property import ComponentProperty
from esdoc_api.lib.pycim.v1_5.types.software.composition import Composition
from esdoc_api.lib.pycim.v1_5.types.software.connection import Connection
from esdoc_api.lib.pycim.v1_5.types.software.connection_endpoint import ConnectionEndpoint
from esdoc_api.lib.pycim.v1_5.types.software.connection_property import ConnectionProperty
from esdoc_api.lib.pycim.v1_5.types.software.coupling import Coupling
from esdoc_api.lib.pycim.v1_5.types.software.coupling_endpoint import CouplingEndpoint
from esdoc_api.lib.pycim.v1_5.types.software.coupling_property import CouplingProperty
from esdoc_api.lib.pycim.v1_5.types.software.deployment import Deployment
from esdoc_api.lib.pycim.v1_5.types.software.entry_point import EntryPoint
from esdoc_api.lib.pycim.v1_5.types.software.model_component import ModelComponent
from esdoc_api.lib.pycim.v1_5.types.software.parallelisation import Parallelisation
from esdoc_api.lib.pycim.v1_5.types.software.processor_component import ProcessorComponent
from esdoc_api.lib.pycim.v1_5.types.software.rank import Rank
from esdoc_api.lib.pycim.v1_5.types.software.spatial_regridding import SpatialRegridding
from esdoc_api.lib.pycim.v1_5.types.software.spatial_regridding_property import SpatialRegriddingProperty
from esdoc_api.lib.pycim.v1_5.types.software.spatial_regridding_user_method import SpatialRegriddingUserMethod
from esdoc_api.lib.pycim.v1_5.types.software.time_lag import TimeLag
from esdoc_api.lib.pycim.v1_5.types.software.time_transformation import TimeTransformation
from esdoc_api.lib.pycim.v1_5.types.software.timing import Timing


# Module exports.
__all__ = ['Activity', 'BoundaryCondition', 'Conformance', 'Ensemble', 'EnsembleMember', 'Experiment', 'ExperimentRelationship', 'ExperimentRelationshipTarget', 'InitialCondition', 'MeasurementCampaign', 'NumericalActivity', 'NumericalExperiment', 'NumericalRequirement', 'OutputRequirement', 'PhysicalModification', 'RequirementOption', 'Simulation', 'SimulationComposite', 'SimulationRelationship', 'SimulationRelationshipTarget', 'SimulationRun', 'SpatioTemporalConstraint', 'DataContent', 'DataDistribution', 'DataExtent', 'DataExtentGeographical', 'DataExtentTemporal', 'DataExtentTimeInterval', 'DataHierarchyLevel', 'DataObject', 'DataProperty', 'DataRestriction', 'DataStorage', 'DataStorageDb', 'DataStorageFile', 'DataStorageIp', 'DataTopic', 'CoordinateList', 'GridExtent', 'GridMosaic', 'GridProperty', 'GridSpec', 'GridTile', 'GridTileResolutionType', 'VerticalCoordinateList', 'CimQuality', 'Evaluation', 'Measure', 'Report', 'Calendar', 'Change', 'ChangeProperty', 'CimDocumentRelationship', 'CimDocumentRelationshipTarget', 'CimGenealogy', 'CimInfo', 'CimReference', 'CimRelationship', 'CimTypeInfo', 'Citation', 'ClosedDateRange', 'Compiler', 'Daily360', 'DataSource', 'DateRange', 'License', 'Machine', 'MachineCompilerUnit', 'OpenDateRange', 'PerpetualPeriod', 'Platform', 'Property', 'RealCalendar', 'ResponsibleParty', 'ResponsiblePartyContactInfo', 'Standard', 'StandardName', 'Component', 'ComponentLanguage', 'ComponentLanguageProperty', 'ComponentProperty', 'Composition', 'Connection', 'ConnectionEndpoint', 'ConnectionProperty', 'Coupling', 'CouplingEndpoint', 'CouplingProperty', 'Deployment', 'EntryPoint', 'ModelComponent', 'Parallelisation', 'ProcessorComponent', 'Rank', 'SpatialRegridding', 'SpatialRegriddingProperty', 'SpatialRegriddingUserMethod', 'TimeLag', 'TimeTransformation', 'Timing']


# Module provenance info.
__author__="Mark Morgan"
__copyright__ = "Copyright 2013 - Institut Pierre Simon Laplace."
__date__ ="2013-01-30 15:45:18.472829"
__license__ = "GPL"
__version__ = "1.5.0"
__maintainer__ = "Mark Morgan"
__email__ = "momipsl@ipsl.jussieu.fr"
__status__ = "Production"

