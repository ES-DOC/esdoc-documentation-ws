"""
.. module:: esdoc_api.lib.pyesdoc.serialization.decoder_xml_utils.py
...
def decode_xml(decoder, xml, nsmap, is_iterable):
    ...
    # otherwise exception
    raise PYESDOCException("xml cannot be decoded.")
...
def load_xml(xml, return_nsmap=False, default_ns='cim'):
    ...
    if xml is None:
        raise PYESDOCException("XML is undefined.")
        ...
        try:
            ...
            if isinstance(xml, basestring):
                try:
                    ...
                    raise PYESDOCException("Invalid xml string.")
            else:
                raise PYESDOCException("Unsupported xml type, must be either a string, file, url or etree.")
