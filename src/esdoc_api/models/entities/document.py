...
class Document(CIMEntity):
    ...
    @classmethod
    def get_names(cls, project, type=None, latest_only=True, childless_only=True):
        ...
        if latest_only == True:
            q = q.filter(cls.IsLatest==True)
        if childless_only == True:
            q = q.filter(cls.HasChildren==False)
        ...
        for document in q.all():
            result.append({
                ...
                'name' : document.Name,
                'type' : document.Type,
                'institute' : None if document.Institute is None else document.Institute.Name
            })
        return result
