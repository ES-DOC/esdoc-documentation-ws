...
_FOLDER_RAW = "raw"
_FOLDER_PARSED = "parsed"
_FOLDER_ERROR = "parsed_error"

# Original encoding type literal.
_ORIGINAL = "original"

# Default document encoding.
_ENCODING = pyesdoc.ESDOC_ENCODING_JSON
...
class _DocumentContextInfo(object):
    ...
    def __init__(self, project, source, fpath, index, verbose):
        ...
        self.index = index
        self.fpath = fpath
        self.fpath_parsed = fpath.replace(_FOLDER_RAW, _FOLDER_PARSED) \
                                  .replace(self.encoding, _ENCODING)
        self.fpath_error = fpath.replace(_FOLDER_RAW, _FOLDER_ERROR)
...
def _decode(ctx):
    """Decodes a document."""
    try:
        ctx.decoded = pyesdoc.read(ctx.fpath, encoding=ctx.encoding)
...
def _parse(ctx):
    """Parses a document."""
    doc_parser.parse(ctx.project, ctx.source, ctx.decoded)
    pyesdoc.extend(ctx.decoded)


def _write(ctx):
    """Writes a parsed document to the file system."""
    try:
        pyesdoc.write(ctx.decoded, _ENCODING, ctx.fpath_parsed)
...
def _organize(ctx):
    ...
    dirname = ctx.dirname.replace(_FOLDER_RAW, _FOLDER_ORGANIZED)
    fname = "{0}_{1}_{2}".format(
        ctx.decoded.meta.type.replace(".", "-").lower(),
        ctx.decoded.meta.id,
        ctx.decoded.meta.version,
        )
...
def _get_documents(throttle, verbose):
    ...
    scanned = 0
    yielded = 0
    for project, source, fpath in io.get_documents(_FOLDER_RAW):
        scanned = scanned + 1
...
def _process(ctx):
    ...
    rt.invoke(ctx,
              (_log_start, _decode, _parse, _write, _organize),
