...
def get_ingest_state(item_id=None):
    """Returns either all ingest states or first ingest state with matching name.

    :param item_id: Ingest state identifier.
    :type item_id: str | int

    :returns: Either an ingest state list or an instance.
    :rtype: list | models.IngestState

    """
    return core.get(models.IngestState, item_id)
